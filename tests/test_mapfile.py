...
def test_raster_colorCoding():
    clsstr = colorCoding('raster',2,10,[[100,255,0],'100 255 50','#000000'])
    assert clsstr.find('#64ff32') > -1
    clsstr = colorCoding('raster',2,2,['a','b'])
    assert clsstr.find("'2'") > -1
    clsstr = colorCoding('raster',2,5,['a'])
    assert clsstr.find('2 - 5') > -1
    clsstr = colorCoding('raster',10,2,['a','b'])
    assert clsstr == ""
    clsstr = colorCoding('raster',2,10,{"classes":['a','b'],"name":"foo"})
    assert clsstr.find('foo') > -1
    clsstr = colorCoding('raster',2,10,{'name': 'absolute',"classes":[
    ...
    clsstr = colorCoding('raster',2,10,{'classes': [
